...
function extractNameFromTemplate(value) {
  const startPos = 'Hello, '.length;
  return value.slice(startPos, -1);
}
...
function encodeToRot13(str) {
  const baseCap = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const baseLow = 'abcdefghijklmnopqrstuvwxyz';
  return str
    .split('')
    .map((char) => {
      if (baseCap.indexOf(char) !== -1) {
        return baseCap.at(baseCap.indexOf(char) - 13);
      }
      if (baseLow.indexOf(char) !== -1) {
        return baseLow.at(baseLow.indexOf(char) - 13);
      }
      return char;
    })
    .join('');
